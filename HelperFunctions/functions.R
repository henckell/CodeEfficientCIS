...
testGraph <- function(n,reps,epsilon,alpha,x,y,Z,C,amat,error="Gaussian",complex=TRUE){
  ...
  betas_liml <- matrix(numeric(reps*k),ncol=k)
  betas_jive <- matrix(numeric(reps*k),ncol=k)
  ...
  for(i in 1:reps){
    ...
    for(j in 1:length(Z)){
      ...
      if(complex==TRUE){
        if(length(Z[[j]])!=0){
          if(length(C[[j]])!=0){
      betas_liml[i,j] <- LIML(ivmodel(Y=Data[,y],D=Data[,x],Z=Data[,Z[[j]]],X=Data[,C[[j]]]))$point.est
      betas_jive[i,j] <- jive.est(y=Data[,y],
                                  X=Data[,union(x,C[[j]])],Z=Data[,union(Z[[j]],C[[j]])])$est[1]
      }else{
        betas_liml[i,j] <- LIML(ivmodel(Y=Data[,y],D=Data[,x],Z=Data[,Z[[j]]]))$point.est
        betas_jive[i,j] <- jive.est(y=Data[,y],
                                    X=as.matrix(Data[,union(x,C[[j]])]),Z=as.matrix(Data[,union(Z[[j]],C[[j]])]))$est[1]
                                  }
          }
        }
    }
  ...
  bias <- apply(betas-causalEffect(t(B),y,x,numeric(0)),2,mean)
  ...
  bias_liml <- apply(betas_liml-causalEffect(t(B),y,x,numeric(0)),2,mean)
  var_liml <- apply(betas_liml,2,stats::var)
  MSE_liml <- apply((betas_liml-causalEffect(t(B),y,x,numeric(0)))^2,2,mean)

  bias_jive <- apply(betas_jive-causalEffect(t(B),y,x,numeric(0)),2,mean)
  var_jive <- apply(betas_jive,2,stats::var)
  MSE_jive <- apply((betas_jive-causalEffect(t(B),y,x,numeric(0)))^2,2,mean)

  if(complex==TRUE){
    return(list(tsls=rbind(MSE,avar,var,bias,n),
                liml=rbind(MSE_liml,var_liml,bias_liml,n),
                jive=rbind(MSE_jive,var_jive,bias_jive,n)))
  }
  return(rbind(MSE,avar,var,bias,n))
}

plot_MSE_comparison <- function(MSE, comparison, name_comparison,title=c(""),xtitle=c(""),ytitle=c("RMSE ratio"),bw="nrd0"){
  ...
  p <-  ggplot(df, aes(x=factor(x),y=y),fill=factor(x)) + geom_violin(scale="width",bw=bw) +
